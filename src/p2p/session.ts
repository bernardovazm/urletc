...
type Envelope =
  ...
  | { t: 'fend'; fileId: string }
  | { t: 'fneed'; fileId: string; missing: number[] }
...
interface Incoming {
  ...
  peerId: string // who to ask for the missing pieces; only they may complete this file
  name: string
  ftype: string
  size: number
  total: number
...
export async function joinRoomSession(opts: {
  ...
  /** Ask the sender for the pieces that never landed. False when there is nobody left to
   *  ask, nothing to ask for, or the file has already used up its repair rounds. */
  async function requestRepair(fileId: string): Promise<boolean> {
    const inc = incoming.get(fileId)
    if (!inc || inc.repairs >= MAX_REPAIR_ROUNDS) return false
    const st = peers.get(inc.peerId)
    if (!st?.channel) return false
    const missing: number[] = []
    for (let i = 0; i < inc.total && missing.length < MAX_REPAIR_CHUNKS; i++) if (!inc.chunks[i]) missing.push(i)
    if (!missing.length) return false
    inc.repairs++
    await msg.send(await st.channel.seal(enc.encode(JSON.stringify({ t: 'fneed', fileId, missing } satisfies Envelope))), { target: inc.peerId })
    return true
  }
  ...
  /** (Re)arm the silence watchdog for one incoming file. */
  function watchIncoming(fileId: string): void {
    const inc = incoming.get(fileId)
    if (!inc) return
    if (inc.timer) clearTimeout(inc.timer)
    inc.timer = setTimeout(() => {
      void requestRepair(fileId).then((asked) => (asked ? watchIncoming(fileId) : abandonIncoming(fileId)))
    }, FILE_STALL_MS)
  }

  /** Re-encrypt and re-send one chunk to one peer. Same AAD binding as the first pass. */
  async function resendChunk(out: Outgoing, fileId: string, i: number, peerId: string): Promise<void> {
    const buf = new Uint8Array(await out.file.slice(i * CHUNK, (i + 1) * CHUNK).arrayBuffer())
    const iv = randomBytes(12)
    const ct = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv, additionalData: enc.encode(`${fileId}:${i}`) }, out.key, buf))
    await fdata.send({ fileId, i, iv: bytesToB64(iv), ct: bytesToB64(ct) }, { target: peerId })
  }
  ...
  msg.onMessage = async (data, ctx) => {
    ...
    if (env.t === 'chat') {
      ...
      incoming.set(env.fileId, {
        id: env.fileId,
        from: st.info.deviceId,
        peerId: ctx.peerId,
        name: fname,
        ftype,
        size: fsize,
      ...
      // good unless it is asked for, so this is where a truncated transfer is caught
      // rather than 20 seconds later.
      const inc = incoming.get(String(env.fileId ?? ''))
      if (!inc || inc.peerId !== ctx.peerId || inc.received >= inc.total) return
      ...
      for (const i of missing) {
        ...
        try {
          await resendChunk(out, String(env.fileId), i, ctx.peerId)
        } catch {
    ...
    async sendFile(file: File, skipKeys?: ReadonlySet<string>) {
      ...
      for (const [peerId, st] of ready) {
        ...
        try {
          ...
          recipients.push([peerId, st])
        } catch {
          /* gone before the offer landed; nothing to send them */
        }
      }
