...
export async function mountConsole(app: HTMLElement, caps: CryptoCaps): Promise<void> {
  ...
  /** Per tier, and one tier's failure must not swallow the others: these run in sequence
   *  so a device reachable on two tiers is not sent the same megabytes twice, which also
   *  meant a throw on the first tier silently cancelled the file for every later one. */
  async function sendFileAll(file: File): Promise<number> {
    ...
    for (const tier of BROADCAST_TIERS) {
      ...
      if (!s) continue
      try {
        for (const k of await s.sendFile(file, sent)) sent.add(k)
      } catch (e) {
        failed.push(`${tier}: ${(e as Error).message}`)
      }
    }
    if (failed.length && !sent.size) throw new Error(failed.join('; '))
    if (failed.length) sys(`Some devices did not get "${file.name}" (${failed.join('; ')}).`)
    return sent.size
  }
  ...
  const jumpToLatest = () => {
    // The sheet neutralizes CSS transitions under prefers-reduced-motion, but a scroll asked
    // for in script carries its own behaviour and would ignore that, so it is read here.
    feed.scrollTo({ top: feed.scrollHeight, behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' })
  }
  const jumpDock = el('div', { class: 'jump-dock' }, [button('Jump to latest', jumpToLatest, 'jump-latest', 'Scroll the feed back to the newest item')])
  feed.append(jumpDock)
  /** Distance from the bottom still counted as "at the bottom". Sub-pixel layout and the
   *  feed's own bottom padding both leave a remainder that is not a user scrolling away. */
  const AT_BOTTOM_PX = 24
  const syncJumpLatest = () => {
  ...
  // Every feed item (card, message, even system lines) can be removed with its own delete
  ...
  const addCard = (node: HTMLElement, onRemove?: () => void, place: 'top' | 'bottom' = 'bottom') => {
    ...
    // Two-state control, so it is exposed as one: a screen reader announces the pressed
  ...
  async function clipboardCard(d: Detected): Promise<HTMLElement> {
    ...
    if (d.kind === 'image') {
      ...
      const st = sendTextBtn(() => d.text ?? '')
      if (st) actions.append(st)
      body.append(el('div', { class: 'url-paste-link', text: d.text ?? '' }), verdict, actions)
  ...
  async function launchTool(m: ToolManifest) {
    ...
    // No copy-link button: on a phone it landed on top of the card's delete control, and
    // shared history already carries a tool card to the other device. #/t/<id> deep links
    // still work; only the button is gone.
    const { card, body } = collapsibleCard([el('strong', { text: `${m.icon ?? '🔧'} ${m.name}` })])
    // Which tool this card is, in the DOM. The clipboard boot/refocus path has to be able to
    // ask whether a Clipboard card is already open before it considers opening another.
    card.dataset.tool = m.id
  ...
  if (vviewport) {
    ...
    const applyKeyboardInset = () => {
      // Pinch zoom shrinks the visual viewport as well, and insetting for that would shrink
      // the page underneath the user's own zoom, so only an unzoomed viewport is occlusion.
      const occluded = vviewport.scale > 1.01 ? 0 : Math.max(0, Math.round(window.innerHeight - vviewport.height - vviewport.offsetTop))
      if (occluded === written) return // resize and scroll both fire per frame while a keyboard animates
      written = occluded
      // Per-property CSSOM, never a style attribute: `style-src 'self'` blocks the attribute
      // and permits this.
      root.style.setProperty('--kb-inset', `${occluded}px`)
    }
    vviewport.addEventListener('resize', applyKeyboardInset)
    vviewport.addEventListener('scroll', applyKeyboardInset)
    applyKeyboardInset()
    // The console mounts once for the life of the page, so unload is its teardown: the
    // listeners and the property come off with it rather than outliving the shell they inset
    // (a bfcache restore re-runs mount and would otherwise stack a second pair).
    window.addEventListener(
      'pagehide',
      () => {
        vviewport.removeEventListener('resize', applyKeyboardInset)
        vviewport.removeEventListener('scroll', applyKeyboardInset)
   ...
   *
   *  The Clipboard tool exports the same gate, and this is deliberately not that import: the
   *  tool is a lazy chunk, and naming any of its exports here would pull the whole module
   *  into the entry bundle that only wanted eleven lines of permission check. */
  const readClipboardIfGranted = async (): Promise<Detected[]> => {
    try {
      const perms = navigator.permissions as { query?: (d: { name: string }) => Promise<{ state: string }> } | undefined
      if (!perms?.query || !navigator.clipboard?.read) return []
      if ((await perms.query({ name: 'clipboard-read' }).catch(() => null))?.state !== 'granted') return []
      return await detectItems(await navigator.clipboard.read())
    } catch {
      return [] // blocked or unsupported; the paste listener covers it
    }
  }
  /** What the last read found, so a refocus can tell new content from the content already on
   *  screen. A blob is identified by its size and type rather than its bytes: two different
   *  images of the same size and type read as unchanged, which costs a card that would have
  ...
  const openClipboardTool = async (items: Detected[]) => {
    lastClipboardSig = clipboardSignature(items)
    if (!clipboardTool || feedInner.querySelector('details.card[data-tool="clipboard"]')) return
    await launchTool(clipboardTool)
  }
  ...
  window.addEventListener('focus', () => {
    void (async () => {
      ...
      if (!items.length) return
      if (clipboardSignature(items) === lastClipboardSig) return
      await openClipboardTool(items)
    })()
  })
  if (!p2pReady) return
