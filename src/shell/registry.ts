...
export interface ToolModule {
  ...
  // cached ES-module singleton, so several open cards share one ToolModule object. A tool
  // that owns per-card resources must key them by `container` rather than module scope, so
  // this tears down the exact instance being removed. Older no-arg impls stay valid.
...
/**
 * The capability facade is the security boundary. For built-in (host-realm) tools it is a
 * direct object; for sandboxed Workshop tools it will be the postMessage RPC surface
...
class Registry {
  ...
  register(manifest: ToolManifest): void {
    // Storage-namespace invariant: ids must be delimiter-free so `tool:<id>:<key>`
    // prefixes can never collide across tools (store.ts).
